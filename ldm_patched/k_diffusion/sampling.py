...
def sample_sure_attention(model, x, sigmas, extra_args=None, callback=None, disable=None,
                           ...
                           sure_attn_blocks='all'):
    """DPM-Solver++(2M) with Attention-Guided SURE (SURE-AG) trajectory correction.

    Extends sample_dpmpp_2m_sure by weighting the SURE correction with per-layer
    self-attention entropy.  At each correction step:

      1. Run the denoiser with attention-capture hooks (replaces attn1 with
         attention_basic_with_sim to obtain the full softmax weight matrix).
      2. Compute per-query entropy  H_l[i] = −∑_j A_l[i,j]·log(A_l[i,j])
         for every captured self-attention layer l.
      3. Upsample and average entropy maps across layers → U ∈ [0,1]^(B×1×H×W).
      4. Apply weighted correction:
           x0_corrected = x0_hat − α · (1 + w·U) · ∇SURE(x0_hat)
         where w = sure_attn_weight.

    High-entropy spatial locations (model is uncertain where to attend) receive
    stronger SURE correction; confident low-entropy locations receive less.

    Formal proofs in SURE_attention.lean:
      §2  attn_weight_bounded     — W(U) ∈ [1, 1+w]; correction stays finite
      §3  sure_ag_effective_range — alpha < 1/(2(1+w)) ensures descent
      §4  sure_ag_proxy_descent   — proxy SURE strictly decreases per step
    ...
    sure_n_mc         : Monte Carlo samples for Hutchinson trace (default 1)
    sure_eps          : finite-difference epsilon (default 1e-3)
    sure_preheat_steps: plain-denoise steps before SURE; -1 = auto (15% of steps)
    sure_jac_interval : full Jacobian every N correction steps; -1 = adaptive
    sure_grad_mode    : 'approx' (default, no backward) | 'vjp' | 'full'
    sure_attn_weight  : attention entropy modulation strength w ≥ 0 (default 1.0).
                        0 = same as base SURE.  Lean §3: require alpha < 1/(2(1+w)).
    sure_attn_blocks  : UNet blocks to capture entropy from:
                        'all' (default) | 'middle' | 'mid+out' | 'input' | 'output'
                        'middle' is cheapest (~5 MiB extra); 'all' is most informative.
    """
    from ldm_patched.k_diffusion.sure_attention import _sure_correct_x0_attention
    ...
    _adam_state = (
        ...
        if sure_adam_mode in ('adam', 'adamw') else None
    )
    _swap_hist = model_management.should_swap_history()
    _old_buf   = None

    # Lean §3 advisory: alpha must satisfy alpha*(1+w) < 1/2 for guaranteed descent.
    _alpha_max = 0.5 / (1.0 + max(sure_attn_weight, 0.0))
    if sure_alpha >= _alpha_max:
        _sure_logger.warning(
            "[SURE-AG] sure_alpha=%.4f >= alpha_max=%.4f (1/(2*(1+w))); "
            "descent not guaranteed (Lean sure_ag_effective_range). "
            "Consider sure_alpha < %.4f.",
            sure_alpha, _alpha_max, _alpha_max,
        )

    _sure_logger.info(
        "DPM++2M-SURE-AG: %d steps  preheat=%d  alpha=%.4f  "
        "attn_weight=%.2f  attn_blocks=%s  grad_mode=%s",
        n_steps, preheat, sure_alpha,
        sure_attn_weight, sure_attn_blocks, sure_grad_mode,
    )
    ...
    for i in trange(n_steps, disable=disable):
        ...
        if i < preheat:
            ...
            denoised, _stats = _sure_correct_x0_attention(
                model, x0_hat, sigma_hat_0, s_in, extra_args,
                alpha=sure_alpha, n_mc=sure_n_mc, eps_mc=sure_eps,
                use_jac=_use_jac, sigma_t=sigma,
                adam_state=_adam_state, adam_mode=sure_adam_mode,
                adam_beta1=sure_adam_beta1, adam_beta2=sure_adam_beta2,
                adam_wd=sure_adam_wd, grad_mode=sure_grad_mode,
                attn_weight=sure_attn_weight,
                attn_blocks=sure_attn_blocks,
            )
            _corr_count += 1
        ...
        old_denoised = (
            _old_buf.to_device()
            if isinstance(_old_buf, model_management.CPUSwapBuffer)
            else _old_buf
        )
