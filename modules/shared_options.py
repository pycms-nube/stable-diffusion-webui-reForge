...
options_templates.update(options_section(('sampler-params', "reForge Sampler Parameters", "sd"), {
    ...
    "sure_grad_mode": OptionInfo("vjp", "SURE - grad_mode (gradient computation)", gr.Radio, {"choices": ["approx", "vjp", "full"]}, infotext='SURE grad_mode').info('approx = stop-grad 2·r (fast, no backward); vjp = exact ∇‖x−x̂‖² via one backward pass (default); full = full ∇SURE including MC Jacobian-trace gradient (2 extra backward passes, requires jac enabled)'),
