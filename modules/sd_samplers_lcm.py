import torch
from modules.sd_sampling_backend import get_sampling, get_external, get_k_diffusion

from modules import shared, sd_samplers_cfg_denoiser, sd_samplers_kdiffusion, sd_samplers_common


_lcm_denoiser_class = None

def _get_lcm_denoiser_class():
    global _lcm_denoiser_class
    if _lcm_denoiser_class is not None:
        return _lcm_denoiser_class

    DiscreteEpsDDPMDenoiser = get_external().DiscreteEpsDDPMDenoiser
    sampling = get_sampling()
    utils = get_k_diffusion().utils
    ...
    _lcm_denoiser_class = LCMCompVisDenoiser
    return _lcm_denoiser_class
...
def sample_lcm(model, x, sigmas, extra_args=None, callback=None, disable=None, noise_sampler=None):
    ...
    for i in sampling.trange(len(sigmas) - 1, disable=disable):
...
class CFGDenoiserLCM(sd_samplers_cfg_denoiser.CFGDenoiser):
    ...
    def inner_model(self):
        if self.model_wrap is None:
            self.model_wrap = _get_lcm_denoiser_class()(shared.sd_model)
