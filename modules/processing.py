...
def setup_color_correction(image) -> cv2.Mat | np.ndarray[Any, np.dtype[np.integer[Any] | np.floating[Any]]]:
    logging.info("Calibrating color correction.")
    correction_target: cv2.Mat | np.ndarray[Any, np.dtype[np.integer[Any] | np.floating[Any]]] = cv2.cvtColor(np.asarray(image.copy()), cv2.COLOR_RGB2LAB)
    return correction_target


def apply_color_correction(correction, original_image) -> Image.Image:
    logging.info("Applying color correction.")
    image: Image.Image = Image.fromarray(cv2.cvtColor(exposure.match_histograms(
        cv2.cvtColor(
    ...
    image: Image.Image = blendLayers(image, original_image, BlendType.LUMINOSITY)

    return image.convert('RGB')


def uncrop(image, dest_size, paste_loc) -> Image.Image:
    x, y, w, h = paste_loc
    base_image: Image.Image = Image.new('RGBA', dest_size)
    image = images.resize_image(1, image, w, h)
    base_image.paste(image, (x, y))
    image: Image.Image = base_image
...
def apply_overlay(image, paste_loc, overlay):
    ...
    if paste_loc is not None:
        image: Image.Image = uncrop(image, (overlay.width, overlay.height), paste_loc)
...
def txt2img_image_conditioning(sd_model, x, width, height):
    if sd_model.model.conditioning_key in {'hybrid', 'concat'}: # Inpainting models
        ...
        image_conditioning: torch.Tensor = torch.ones(x.shape[0], 3, height, width, device=x.device) * 0.5
        image_conditioning = images_tensor_to_samples(image_conditioning, approximation_indexes.get(shared.opts.sd_vae_encode_method))

        # Add the fake full 1s mask to the first dimension.
        image_conditioning: torch.Tensor = torch.nn.functional.pad(image_conditioning, (0, 0, 0, 0, 1, 0), value=1.0)
        image_conditioning: torch.Tensor = image_conditioning.to(x.dtype)
...
class StableDiffusionProcessing:
    ...
    cached_uc: ClassVar[list[None]] = [None, None, None]
    cached_c: ClassVar[list[None]] = [None, None, None]
    ...
    def __post_init__(self) -> None:
        ...
        self.cached_uc: list[None] = StableDiffusionProcessing.cached_uc
        self.cached_c: list[None] = StableDiffusionProcessing.cached_c
    ...
    def fill_fields_from_opts(self) -> None:
    ...
    @property
    def scripts(self) -> scripts.ScriptRunner:
        return self.scripts_value

    @scripts.setter
    def scripts(self, value) -> None:
    ...
    @script_args.setter
    def script_args(self, value) -> None:
    ...
    def depth2img_image_conditioning(self, source_image) -> torch.Tensor:
        # Use the AddMiDaS helper to Format our source image to suit the MiDaS model
        transformer = AddMiDaS(model_type="dpt_hybrid")
        transformed = transformer({"jpg": rearrange(source_image[0], "c h w -> h w c")})
        midas_in: torch.Tensor = torch.from_numpy(transformed["midas_in"][None, ...]).to(device=shared.device)
        midas_in: torch.Tensor = repeat(midas_in, "1 ... -> n ...", n=self.batch_size)

        conditioning_image = images_tensor_to_samples(source_image*0.5+0.5, approximation_indexes.get(shared.opts.sd_vae_encode_method))
        conditioning: torch.Tensor = torch.nn.functional.interpolate(
        ...
        conditioning: torch.Tensor = 2. * (conditioning - depth_min) / (depth_max - depth_min) - 1.
        return conditioning
    ...
    def unclip_image_conditioning(self, source_image):
        ...
        if self.sd_model.noise_augmentor is not None:
            ...
            c_adm: torch.Tensor = torch.cat((c_adm, noise_level_emb), 1)
        return c_adm
    ...
    def inpainting_image_conditioning(self, source_image, latent_image, image_mask=None, round_image_mask=True):
        ...
        if image_mask is not None:
            if torch.is_tensor(image_mask):
                conditioning_mask: torch.Tensor = image_mask
            else:
                conditioning_mask = np.array(image_mask.convert("L"))
                conditioning_mask = conditioning_mask.astype(np.float32) / 255.0
                conditioning_mask: torch.Tensor = torch.from_numpy(conditioning_mask[None, None])

                if round_image_mask:
                    # Caller is requesting a discretized mask as input, so we round to either 1.0 or 0.0
                    conditioning_mask: torch.Tensor = torch.round(conditioning_mask)
        ...
        conditioning_mask: torch.Tensor = torch.nn.functional.interpolate(conditioning_mask, size=latent_image.shape[-2:])
        conditioning_mask: torch.Tensor = conditioning_mask.expand(conditioning_image.shape[0], -1, -1, -1)
        image_conditioning: torch.Tensor = torch.cat([conditioning_mask, conditioning_image], dim=1)
    ...
    def init(self, all_prompts, all_seeds, all_subseeds) -> None:
        pass
    ...
    def close(self) -> None:
        ...
        if not shared.opts.persistent_cond_cache:
            StableDiffusionProcessing.cached_c: list[None] = [None, None]
            StableDiffusionProcessing.cached_uc: list[None] = [None, None]
    ...
    def cached_params(self, required_prompts, steps, extra_network_data, hires_steps=None, use_old_scheduling=False):
        ...
        return (
            ...
            hires_steps,
            use_old_scheduling,
            shared.opts.CLIP_stop_at_last_layers,
            shared.opts.dynamic_clip_skip_enabled,
            shared.opts.dynamic_clip_skip_start,
            shared.opts.dynamic_clip_skip_use_schedule,
            shared.opts.dynamic_clip_skip_schedule_values,
            shared.sd_model.sd_checkpoint_info,
            extra_network_data,
            shared.opts.sdxl_crop_left,
            shared.opts.sdxl_crop_top,
            self.width,
            self.height,
            shared.opts.fp8_storage,
            shared.opts.cache_fp16_weight,
            shared.opts.emphasis,
        )

    def _minimal_clip_skip(self) -> Any | int:
        model: WebuiSdModel = self.sd_model or shared.sd_model
        cond_model: Any | None = getattr(model, "cond_stage_model", None) if model is not None else None
    ...
    def _parse_dynamic_schedule(self, total_steps, minimal_clip_skip):
        raw = shared.opts.dynamic_clip_skip_schedule_values or ""
        parts = [p.strip() for p in raw.replace(";", ",").split(",") if p.strip() != ""]
        values = []
        for p in parts:
            try:
                v = int(float(p))
                v: int = max(v, 2, minimal_clip_skip)
                v: int = min(v, 12)
                values.append(v)
            except Exception:
                continue
    ...
    def setup_conds(self) -> None:
        prompts: prompt_parser.SdConditioning[list] = prompt_parser.SdConditioning(self.prompts, width=self.width, height=self.height)
        negative_prompts: prompt_parser.SdConditioning[list] = prompt_parser.SdConditioning(self.negative_prompts, width=self.width, height=self.height, is_negative_prompt=True)

        sampler_config: sd_samplers_common.SamplerData | None = sd_samplers.find_sampler_config(self.sampler_name)
        total_steps: int = sampler_config.total_steps(self.steps) if sampler_config else self.steps
        self.step_multiplier = total_steps // self.steps
        self.firstpass_steps: int = total_steps

        self.dynamic_clip_skip_schedule = None
        self.dynamic_clip_skip_sets = None

        if shared.opts.dynamic_clip_skip_enabled:
            minimal_clip_skip = self._minimal_clip_skip()
            schedule = None
            custom_schedule_used = False

            if shared.opts.dynamic_clip_skip_use_schedule:
                schedule = self._parse_dynamic_schedule(total_steps, minimal_clip_skip)
                custom_schedule_used: bool = schedule is not None

            if schedule is None:
                start_clip_skip: int = max(int(shared.opts.dynamic_clip_skip_start), 2, minimal_clip_skip)
                start_clip_skip: int = min(start_clip_skip, 12)
                schedule = self._build_dynamic_clip_skip_schedule(total_steps, start_clip_skip)
            else:
                start_clip_skip = schedule[0] if schedule else max(int(shared.opts.dynamic_clip_skip_start), 2, minimal_clip_skip)

            unique_clip_skips = list(dict.fromkeys(schedule))

            original_clip_skip = shared.opts.CLIP_stop_at_last_layers
            cond_sets = {}
            try:
                ...
                shared.opts.CLIP_stop_at_last_layers = original_clip_skip
            ...
            if custom_schedule_used:
                self.extra_generation_params["Dynamic clip skip schedule"] = shared.opts.dynamic_clip_skip_schedule_values or ",".join(map(str, schedule))
            else:
    ...
    def parse_extra_network_prompts(self) -> None:
        self.prompts, self.extra_network_data = extra_networks.parse_prompts(self.prompts)

    def save_samples(self) -> bool:
        """Returns whether generated images need to be written to disk"""
        return shared.opts.samples_save and not self.do_not_save_samples and (shared.opts.save_incomplete_images or not shared.state.interrupted and not shared.state.skipped)
...
class Processed:
    def __init__(self, p: StableDiffusionProcessing, images_list, seed=-1, info="", subseed=None, all_prompts=None, all_negative_prompts=None, all_seeds=None, all_subseeds=None, index_of_first_image=0, infotexts=None, comments="", extra_images_list=[]) -> None:
        ...
        self.prompt: str = p.prompt
        self.negative_prompt: str = p.negative_prompt
        self.seed: int = seed
        self.subseed = subseed
        self.subseed_strength: float = p.subseed_strength
        self.info: str = info
        self.comments: str = "".join(f"{comment}\n" for comment in p.comments)
        self.width: int = p.width
        self.height: int = p.height
        self.sampler_name: str = p.sampler_name
        self.cfg_scale: float = p.cfg_scale
        self.image_cfg_scale: Any | None = getattr(p, 'image_cfg_scale', None)
        self.steps: int = p.steps
        self.batch_size: int = p.batch_size
        self.restore_faces: bool = p.restore_faces
        self.face_restoration_model = shared.opts.face_restoration_model if p.restore_faces else None
        self.sd_model_name: str = p.sd_model_name
        self.sd_model_hash: str = p.sd_model_hash
        self.sd_vae_name: str = p.sd_vae_name
        self.sd_vae_hash: str = p.sd_vae_hash
        self.seed_resize_from_w: int = p.seed_resize_from_w
        self.seed_resize_from_h: int = p.seed_resize_from_h
        self.denoising_strength: Any | None = getattr(p, 'denoising_strength', None)
        self.extra_generation_params: dict[str, Any] = p.extra_generation_params
        self.index_of_first_image: int = index_of_first_image
        self.styles: list[str] = p.styles
        self.job_timestamp: str = shared.state.job_timestamp
        self.clip_skip = shared.opts.CLIP_stop_at_last_layers
        self.token_merging_ratio: int = p.token_merging_ratio
        self.token_merging_ratio_hr: int = p.token_merging_ratio_hr

        self.eta: float = p.eta
        self.ddim_discretize: str = p.ddim_discretize
        self.s_churn: float = p.s_churn
        self.s_tmin: float = p.s_tmin
        self.s_tmax: float = p.s_tmax
        self.s_noise: float = p.s_noise
        self.s_min_uncond: float = p.s_min_uncond
        self.sampler_noise_scheduler_override: None = p.sampler_noise_scheduler_override
        self.prompt: str = self.prompt if not isinstance(self.prompt, list) else self.prompt[0]
        self.negative_prompt: str = self.negative_prompt if not isinstance(self.negative_prompt, list) else self.negative_prompt[0]
        self.seed: int = int(self.seed if not isinstance(self.seed, list) else self.seed[0]) if self.seed is not None else -1
        self.subseed: int = int(self.subseed if not isinstance(self.subseed, list) else self.subseed[0]) if self.subseed is not None else -1
        self.is_using_inpainting_conditioning: bool = p.is_using_inpainting_conditioning
        ...
        self.version: None | str = program_version()
    ...
    def get_token_merging_ratio(self, for_hr=False) -> int:
        return self.token_merging_ratio_hr if for_hr else self.token_merging_ratio


def create_random_tensors(shape, seeds, subseeds=None, subseed_strength=0.0, seed_resize_from_h=0, seed_resize_from_w=0, p=None) -> torch.Tensor:
    g: rng.ImageRNG = rng.ImageRNG(shape, seeds, subseeds=subseeds, subseed_strength=subseed_strength, seed_resize_from_h=seed_resize_from_h, seed_resize_from_w=seed_resize_from_w)
    return g.next()
...
def decode_latent_batch(model, batch, target_device=None, check_for_nans=False) -> DecodedSamples:
...
def program_version() -> None | str:
    import launch

    res: str = launch.git_tag()
    if res == "<none>":
        res = None
...
def create_infotext(p, all_prompts, all_seeds, all_subseeds, comments=None, iteration=0, position_in_batch=0, use_main_prompt=False, index=None, all_negative_prompts=None):
    ...
    enable_hr: Any | bool = getattr(p, 'enable_hr', False)
    ...
    generation_params = {
        ...
        "Model hash": p.sd_model_hash if shared.opts.add_model_hash_to_info else None,
        "Model": p.sd_model_name if shared.opts.add_model_name_to_info else None,
        "FP8 weight": shared.opts.fp8_storage if devices.fp8 else None,
        "Cache FP16 weight for LoRA": shared.opts.cache_fp16_weight if devices.fp8 else None,
        "VAE hash": p.sd_vae_hash if shared.opts.add_vae_hash_to_info else None,
        "VAE": p.sd_vae_name if shared.opts.add_vae_name_to_info else None,
        ...
        "RNG": shared.opts.randn_source if shared.opts.randn_source != "GPU" else None,
        "Tiling": "True" if p.tiling else None,
        **p.extra_generation_params,
        "Version": program_version() if shared.opts.add_version_to_infotext else None,
        "User": p.user if shared.opts.add_user_name_to_info else None,
    }
    ...
    generation_params_text: str = ", ".join([k if k == v else f'{k}: {infotext_utils.quote(v)}' for k, v in generation_params.items() if v is not None])

    negative_prompt_text: str = f"\nNegative prompt: {negative_prompt}" if negative_prompt else ""
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    stored_opts = {k: shared.opts.data[k] if k in shared.opts.data else shared.opts.get_default(k) for k in p.override_settings.keys() if k in shared.opts.data}
    ...
    try:
        ...
        for k, v in p.override_settings.items():
            shared.opts.set(k, v, is_api=True, run_callbacks=False)
        ...
        with profiling.Profiler():
            res: Processed = process_images_inner(p)
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    seed: int = get_fixed_seed(p.seed)
    subseed: int = get_fixed_seed(p.subseed)
    ...
    with torch.inference_mode():
        with devices.autocast():
            ...
            if shared.opts.live_previews_enable and shared.opts.show_progress_type == "Approx NN":
                sd_vae_approx.model()
        ...
        for n in range(p.n_iter):
            ...
            latent_channels: Any | int = getattr(shared.sd_model, 'latent_channels', opt_C)
            ...
            p.extra_generation_params.update(modules.sd_hijack.model_hijack.extra_generation_params)
            ...
            if (
                ...
                if not getattr(shared.opts, 'use_old_clip_g_load_and_ztsnr_application', False):
            ...
            if p.scripts is not None:
                ps: scripts.PostSampleArgs = scripts.PostSampleArgs(samples_ddim)
            ...
            if getattr(samples_ddim, 'already_decoded', False):
                ...
                x_samples_ddim: DecodedSamples = decode_latent_batch(p.sd_model, samples_ddim, target_device=devices.cpu, check_for_nans=True)

            x_samples_ddim: torch.Tensor = torch.stack(x_samples_ddim).float()
            x_samples_ddim: torch.Tensor = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
            ...
            if p.scripts is not None:
                ...
                batch_params: scripts.PostprocessBatchListArgs[list[Tensor]] = scripts.PostprocessBatchListArgs(list(x_samples_ddim))
                p.scripts.postprocess_batch_list(p, batch_params, batch_number=n)
                x_samples_ddim: list[Tensor] = batch_params.images
            ...
            def infotext(index=0, use_main_prompt=False):
                if shared.opts.enable_prompt_comments_def:
                    ...
                    clean_prompts: list[str] = [comments_parser.strip_comments(prompt) for prompt in p.prompts]
                    clean_negative_prompts: list[str] = [comments_parser.strip_comments(negative_prompt) for negative_prompt in p.negative_prompts]
            ...
            save_samples: bool = p.save_samples()

            for i, x_sample in enumerate(x_samples_ddim):
                p.batch_index = i

                x_sample = 255. * np.moveaxis(x_sample.cpu().numpy(), 0, 2)
                x_sample: np.ndarray[tuple[Any, ...], np.dtype[np.unsignedinteger[np._8Bit]]] = x_sample.astype(np.uint8)
                ...
                image: Image.Image = Image.fromarray(x_sample)

                if p.scripts is not None:
                    pp: scripts.PostprocessImageArgs[Image.Image] = scripts.PostprocessImageArgs(image)
                    p.scripts.postprocess_image(p, pp)
                    image: Image.Image = pp.image

                mask_for_overlay: Any | None = getattr(p, "mask_for_overlay", None)
                ...
                if p.scripts is not None:
                    ppmo: scripts.PostProcessMaskOverlayArgs[int, Any | None, None | Any] = scripts.PostProcessMaskOverlayArgs(i, mask_for_overlay, overlay_image)
                ...
                if p.color_corrections is not None and i < len(p.color_corrections):
                    ...
                    image: Image.Image = apply_color_correction(p.color_corrections[i], image)
                ...
                if mask_for_overlay is not None:
                    ...
                    if shared.opts.return_mask_composite or shared.opts.save_mask_composite:
                        image_mask_composite: Image.Image = Image.composite(original_denoised_image.convert('RGBA').convert('RGBa'), Image.new('RGBa', image.size), images.resize_image(2, mask_for_overlay, image.width, image.height).convert('L')).convert('RGBA')
        ...
        if (shared.opts.return_grid or shared.opts.grid_save) and not p.do_not_save_grid and not unwanted_grid_because_of_img_count:
            grid: Image.Image = images.image_grid(output_images, p.batch_size)
...
def old_hires_fix_first_pass_dimensions(width, height):
    ...
    scale: float = math.sqrt(desired_pixel_count / actual_pixel_count)
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    cached_hr_uc: ClassVar[list[None]] = [None, None, None]
    cached_hr_c: ClassVar[list[None]] = [None, None, None]
    ...
    def __post_init__(self) -> None:
        ...
        self.cached_hr_uc: list[None] = StableDiffusionProcessingTxt2Img.cached_hr_uc
        self.cached_hr_c: list[None] = StableDiffusionProcessingTxt2Img.cached_hr_c

    def calculate_target_resolution(self) -> None:
        ...
        if self.hr_resize_x == 0 and self.hr_resize_y == 0:
            ...
            if self.hr_resize_y == 0:
                ...
                target_w: int = self.hr_resize_x
                target_h: int = self.hr_resize_y
                src_ratio = self.width / self.height
                dst_ratio: float = self.hr_resize_x / self.hr_resize_y
        ...
        if self.enable_hr:
            ...
            if not shared.state.processing_has_refined_job_count:
                ...
                if getattr(self, 'txt2img_upscale', False):
                    total_steps: int = (self.hr_second_pass_steps or self.steps) * shared.state.job_count
                else:
                    total_steps: int = (self.steps + (self.hr_second_pass_steps or self.steps)) * shared.state.job_count
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        if self.firstpass_image is not None and self.enable_hr:
            ...
            if self.latent_scale_mode is None:
                ...
                image: np.ndarray[tuple[Any, ...], np.dtype[np.floating[Any]]] = np.moveaxis(image, 2, 0)

                samples = None
                decoded_samples: torch.Tensor = torch.asarray(np.expand_dims(image, 0))

            else:
                image = np.array(self.firstpass_image).astype(np.float32) / 255.0
                image: np.ndarray[tuple[Any, ...], np.dtype[np.floating[Any]]] = np.moveaxis(image, 2, 0)
                image: torch.Tensor = torch.from_numpy(np.expand_dims(image, axis=0))
                image: torch.Tensor = image.to(shared.device, dtype=torch.float32)

                if shared.opts.sd_vae_encode_method != 'Full':
                    self.extra_generation_params['VAE Encoder'] = shared.opts.sd_vae_encode_method

                samples = images_tensor_to_samples(image, approximation_indexes.get(shared.opts.sd_vae_encode_method), self.sd_model)
            ...
            if self.latent_scale_mode is None:
                decoded_samples: torch.Tensor = torch.stack(decode_latent_batch(self.sd_model, samples, target_device=devices.cpu, check_for_nans=True)).to(dtype=torch.float32)
            else:
                decoded_samples = None
    ...
    def sample_hr_pass(self, samples, decoded_samples, seeds, subseeds, subseed_strength, prompts):
        ...
        def save_intermediate(image, index) -> None:
            """saves image before applying hires fix, if enabled in options; takes as an argument either an image or batch with latent space images"""

            if not self.save_samples() or not shared.opts.save_images_before_highres_fix:
                return

            if not isinstance(image, Image.Image):
                image: Image.Image = sd_samplers.sample_to_image(image, index, approximation=0)

            info = create_infotext(self, self.all_prompts, self.all_seeds, self.all_subseeds, [], iteration=self.iteration, position_in_batch=index)
            images.save_image(image, self.outpath_samples, "", seeds[index], prompts[index], shared.opts.samples_format, info=info, p=self, suffix="-before-highres-fix")

        img2img_sampler_name: str = self.hr_sampler_name or self.sampler_name
        ...
        if self.latent_scale_mode is not None:
            ...
            samples: torch.Tensor = torch.nn.functional.interpolate(samples, size=(target_height // opt_f, target_width // opt_f), mode=self.latent_scale_mode["mode"], antialias=self.latent_scale_mode["antialias"])
            ...
            lowres_samples: torch.Tensor = torch.clamp((decoded_samples + 1.0) / 2.0, min=0.0, max=1.0)
            ...
            decoded_samples: torch.Tensor = torch.from_numpy(np.array(batch_images))
            decoded_samples: torch.Tensor = decoded_samples.to(shared.device, dtype=torch.float32)

            if shared.opts.sd_vae_encode_method != 'Full':
                self.extra_generation_params['VAE Encoder'] = shared.opts.sd_vae_encode_method
            samples = images_tensor_to_samples(decoded_samples, approximation_indexes.get(shared.opts.sd_vae_encode_method))
        ...
        noise: torch.Tensor = self.rng.next()
        ...
        if self.modified_noise is not None:
            noise: Never = self.modified_noise
            self.modified_noise = None
        ...
        decoded_samples: DecodedSamples = decode_latent_batch(self.sd_model, samples, target_device=devices.cpu, check_for_nans=True)
    ...
    def close(self) -> None:
        ...
        self.hr_uc = None
        if not shared.opts.persistent_cond_cache:
            StableDiffusionProcessingTxt2Img.cached_hr_uc: list[None] = [None, None]
            StableDiffusionProcessingTxt2Img.cached_hr_c: list[None] = [None, None]
    ...
    def calculate_hr_conds(self) -> None:
        if self.hr_c is not None:
            return

        hr_prompts: prompt_parser.SdConditioning[list] = prompt_parser.SdConditioning(self.hr_prompts, width=self.hr_upscale_to_x, height=self.hr_upscale_to_y)
        hr_negative_prompts: prompt_parser.SdConditioning[list] = prompt_parser.SdConditioning(self.hr_negative_prompts, width=self.hr_upscale_to_x, height=self.hr_upscale_to_y, is_negative_prompt=True)

        sampler_config: sd_samplers_common.SamplerData | None = sd_samplers.find_sampler_config(self.hr_sampler_name or self.sampler_name)
        steps: int = self.hr_second_pass_steps or self.steps
        total_steps: int = sampler_config.total_steps(steps) if sampler_config else steps
        ...
        if self.enable_hr:
            if self.hr_cfg < 0 or (self.hr_cfg == 0 and self.cfg_scale == 0):
                ...
                actual_hr_cfg: float = self.cfg_scale
                self.hr_uc = self.get_conds_with_caching(prompt_parser.get_learned_conditioning, hr_negative_prompts, self.firstpass_steps, [self.cached_hr_uc, self.cached_uc], self.hr_extra_network_data, total_steps)
            else:
                actual_hr_cfg: float = self.hr_cfg
                self.hr_uc = self.get_conds_with_caching(prompt_parser.get_learned_conditioning, hr_negative_prompts, self.firstpass_steps, [self.cached_hr_uc, self.cached_uc], self.hr_extra_network_data, total_steps)
    ...
    def parse_extra_network_prompts(self) -> None:
        res: None = super().parse_extra_network_prompts()
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        if image_mask is not None:
            ...
            if self.inpainting_mask_invert:
                image_mask: Image.Image = ImageOps.invert(image_mask)
                self.extra_generation_params["Mask mode"] = "Inpaint not masked"

            if self.mask_blur_x > 0:
                np_mask: np.ndarray[tuple[Any, ...], np.dtype[Any]] = np.array(image_mask)
                kernel_size: int = 2 * int(2.5 * self.mask_blur_x + 0.5) + 1
                np_mask: cv2.Mat | np.ndarray[Any, np.dtype[np.integer[Any] | np.floating[Any]]] = cv2.GaussianBlur(np_mask, (kernel_size, 1), self.mask_blur_x)
                image_mask: Image.Image = Image.fromarray(np_mask)

            if self.mask_blur_y > 0:
                np_mask: np.ndarray[tuple[Any, ...], np.dtype[Any]] = np.array(image_mask)
                kernel_size: int = 2 * int(2.5 * self.mask_blur_y + 0.5) + 1
                np_mask: cv2.Mat | np.ndarray[Any, np.dtype[np.integer[Any] | np.floating[Any]]] = cv2.GaussianBlur(np_mask, (1, kernel_size), self.mask_blur_y)
                image_mask: Image.Image = Image.fromarray(np_mask)
            ...
            if self.inpaint_full_res:
                ...
                mask: Image.Image | Any = image_mask.convert('L')
                crop_region: tuple[int, int, int, int] | None = masking.get_crop_region_v2(mask, self.inpaint_full_res_padding)
                if crop_region:
                    crop_region = masking.expand_crop_region(crop_region, self.width, self.height, mask.width, mask.height)
                    x1, y1, x2, y2 = crop_region
                    mask: Image.Image | Any = mask.crop(crop_region)
                    ...
                    modules.sd_hijack.model_hijack.comments.append(massage)
                    logging.info(massage)
            else:
                image_mask = images.resize_image(self.resize_mode, image_mask, self.width, self.height)
                np_mask: np.ndarray[tuple[Any, ...], np.dtype[Any]] = np.array(image_mask)
                np_mask: np.ndarray[tuple[Any, ...], np.dtype[np.unsignedinteger[np._8Bit]]] = np.clip((np_mask.astype(np.float32)) * 2, 0, 255).astype(np.uint8)
        ...
        for img in self.init_images:
            ...
            if image_mask is not None:
                ...
                image_masked: Image.Image = Image.new('RGBa', (image.width, image.height))
            ...
            if image_mask is not None:
                if self.inpainting_fill != 1:
                    image: Image.Image = masking.fill(image, latent_mask)
        ...
        image: torch.Tensor = torch.from_numpy(batch_images)
        image: torch.Tensor = image.to(shared.device, dtype=torch.float32)
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        if self.mask is not None:
            ...
            if self.scripts is not None:
                mba: scripts.MaskBlendArgs[Any, torch.Tensor, torch.Tensor, Any, Any] = scripts.MaskBlendArgs(samples, self.nmask, self.init_latent, self.mask, blended_samples)
