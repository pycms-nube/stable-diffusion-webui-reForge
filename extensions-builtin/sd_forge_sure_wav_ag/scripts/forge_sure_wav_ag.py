...
class SureWavAGForForge(scripts.Script):
        ...
        with gr.Accordion(open=False, label=self.title()):
            ...
            with gr.Row():
                wavelet = gr.Dropdown(
                    ...
                    value="db4",
                    info="Wavelet family for decomposition.",
                )
                wavelet_level = gr.Slider(
                    label="Wavelet Level",
                    minimum=1, maximum=5, step=1, value=3,
                    info="Decomposition depth. Higher = more frequency bands.",
                )
            with gr.Row():
                lp_frac = gr.Slider(
                    label="LP Fraction",
                ...
                fft_scale = gr.Slider(
                    ...
                    info="FFT low-pass scale on approx gradient (FreeU-style). 1.0=off.",
                )
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        (enabled, alpha, attn_weight, attn_blocks, approx_coeff,
        ...
        unet = opSureWavAG.patch(
            unet, alpha, attn_weight, attn_blocks, approx_coeff,
